...
def find_maximun_subarray(array, low, hight):
    if hight == low:
        return low, hight, array[low]
    else:
        mid = math.floor((low+hight)/2)
        left_low, left_hight, left_sum = find_maximun_subarray(array, low, mid)
        right_low, right_hight, right_sum = find_maximun_subarray(array, mid+1, hight)
